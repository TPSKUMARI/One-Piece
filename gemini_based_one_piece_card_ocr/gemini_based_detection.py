...
GEMINI_PROMPT = """
You are an expert One Piece TCG card reader. You will receive TWO images of the same card:
  IMAGE 1 — The full card front (use this for card_name, language, illustration_desc)
  IMAGE 2 — A cropped close-up of the bottom-right corner (use this for card_number, set_code, rarity)
...
- rarity      : The rarity symbol found in the box immediately to the right of the card number.
                 This is the HARDEST part to read, so follow these instructions VERY CAREFULLY.
...
def call_gemini(image_bytes_1, image_bytes_2=None):
    """
    Send one or two images to Gemini:
      image_bytes_1 — full card image (IMAGE 1, required)
      image_bytes_2 — close-up / second image (IMAGE 2, optional)
    """
    for model in MODELS:
        try:
            parts = [types.Part.from_bytes(data=image_bytes_1, mime_type="image/jpeg")]
            if image_bytes_2:
                parts.append(types.Part.from_bytes(data=image_bytes_2, mime_type="image/jpeg"))
            parts.append(GEMINI_PROMPT.strip())
...
def analyze():
    if "image" not in request.files:
        return jsonify({"success": False, "error": "No image uploaded (field: 'image')"})

    def to_jpeg(file_storage, label):
        raw = file_storage.read()
        img = Image.open(io.BytesIO(raw)).convert("RGB")
        buf = io.BytesIO()
        img.save(buf, format="JPEG", quality=92)
        data = buf.getvalue()
        print(f"  {label} size: {len(data)//1024}KB")
        return data

    try:
        jpeg_bytes_1 = to_jpeg(request.files["image"], "IMAGE 1 (full card)")
    except Exception as e:
        return jsonify({"success": False, "error": f"Invalid image (field 'image'): {e}"})

    jpeg_bytes_2 = None
    if "image2" in request.files and request.files["image2"].filename:
        try:
            jpeg_bytes_2 = to_jpeg(request.files["image2"], "IMAGE 2 (close-up)")
        except Exception as e:
            return jsonify({"success": False, "error": f"Invalid image (field 'image2'): {e}"})

    # Step 1: Gemini extracts card info from both images
    result = call_gemini(jpeg_bytes_1, jpeg_bytes_2)
    if not result["ok"]:
    ...
    if not re.match(r"[A-Z0-9]{2,5}-\d{3,4}", raw_cn):
        ...
        result2 = call_gemini(jpeg_bytes_1, jpeg_bytes_2)
    ...
    best_idx = 0
    print(f"  Returning {len(variant_list)} variant(s), default = index {best_idx}")
